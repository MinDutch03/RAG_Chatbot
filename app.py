...
# Check if cookies are initialized
...
# Loading the chat sessions from cookies if they exist
if "chats" not in cookies:
    cookies["chats"] = {}

# Set up session state from cookie data
st.session_state["chats"] = cookies["chats"]
...
def start_new_chat():
    ...
    cookies["chats"] = st.session_state["chats"]  # Store the chats in cookies
    cookies.save()
...
def reset_current_chat():
    ...
    cookies["chats"] = st.session_state["chats"]  # Update cookies with current chat data
    cookies.save()
...
# Save all updates to cookies at the end of the page load
cookies["chats"] = st.session_state["chats"]
cookies.save()
